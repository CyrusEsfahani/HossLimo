...
const Hero = ({ title, subtitle, buttonText, buttonLink, backgroundVideo, backgroundImage, }) => {
    return (<div className="relative h-screen w-full overflow-hidden">
      {/* Background Video or Image with overlay */}
      <div className="absolute inset-0 bg-black bg-opacity-50 z-10"></div>
      
      {backgroundVideo ? (<video autoPlay muted loop className="absolute top-0 left-0 w-full h-full object-cover scale-105" playsInline // Added for mobile compatibility
        >
          <source src={backgroundVideo} type="video/mp4"/>
          <source src={backgroundVideo.replace('.mp4', '.webm')} type="video/webm"/> {/* Fallback */}
          Your browser does not support the video tag.
        </video>) : backgroundImage ? (<img src={backgroundImage} alt="Hero Background" className="absolute top-0 left-0 w-full h-full object-cover scale-105"/>) : null}
      ...
      <div className="relative z-20 h-full container-custom flex flex-col justify-center items-start text-white">
        <div className="max-w-3xl animate-fadeIn">
          ...
          <react_router_dom_1.Link to={buttonLink} className="inline-block px-8 py-4 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all duration-300 shadow-lg hover:shadow-xl hover:translate-y-[-5px] font-medium text-lg">
            {buttonText}
